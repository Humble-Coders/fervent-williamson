...
import { stylistService, CreateStylistData, getSalonServices } from '../../../../services/stylistService';
// import { Service } from '../../../../services/serviceService'; // Removed unused import
...
const AddStylistPage: React.FC = () => {
  ...
  const loadServices = async () => {
    try {
      logger.info('🔄 Loading services for stylist assignment...');
      const servicesData = await getSalonServices();
      logger.info('📋 Services loaded:', servicesData);

      // Convert services to MultiSelectOption format
      const multiSelectOptions: MultiSelectOption[] = servicesData.map(service => ({
        id: service.id,
        name: service.name,
        description: service.description,
        price: service.price,
        duration: service.duration,
        category: service.categoryId // You might want to map this to actual category name
      }));

      setServices(multiSelectOptions);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await stylistService.createStylist(createData);
      toast.success('Stylist created successfully');
      router.push('/salon/stylists');
    } catch (error: unknown) {
      logger.error('Error creating stylist:', error);
      toast.error((error as { response?: { data?: { message?: string } } })?.response?.data?.message || 'Failed to create stylist');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Add New Stylist</h1>
          <p className="text-gray-600">Add a new team member to your salon</p>
        </div>
      </div>
